document.addEventListener("DOMContentLoaded", () => {
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", (e) => {
      ...
      if (users.find(u => u.email && u.email.toLowerCase() === email)) {
        if (msgEl) {
          msgEl.style.color = "#c00";
          msgEl.textContent = "User already exists! Try logging in.";
        }
        document.getElementById("regEmail").focus();
        return;
      }
  ...
  if (logoutBtn) {
    // global logout function (also usable from onclick attribute)
    window.logout = function () {
      ...
      // Attempt server-side logout endpoints if any; fallback to local finalize
      const attempts = [
  ...
  // Delegated handler for logout action with data-action attribute
  ...
  if (expenseForm) {
    expenseForm.addEventListener("submit", (e) => {
      ...
      if (approvers.length === 0) {
        alert("No approvers exist. Ask Admin to add approvers.");
        return;
      }
  ...
  function renderExpenses() {
    ...
    expenses.forEach((exp, globalIndex) => {
      ...
      if (currentUser.role === "Manager") {
        if (exp.approverEmails && exp.approverEmails.includes(currentUser.email)) {
          if (!mgrBody) return;
          const tr = document.createElement("tr");
          tr.innerHTML = `
            <td>${globalIndex + 1}</td>
            ...
            <td>${exp.status}</td>
            <td>
              ${exp.status === "Pending" ? `<button class="approveBtn" data-index="${globalIndex}">Approve</button> <button class="rejectBtn" data-index="${globalIndex}">Reject</button>` : ""}
            </td>
          `;
  ...
  function handleApproveClick(e) {
    ...
    if (Number.isInteger(idx) && expenses[idx]) {
